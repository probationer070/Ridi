...
def load_audio(file, sr):
    try:
        ...
        file = clean_path(file)  # Prevent newbies from copying the path with spaces, " and carriage returns at the beginning and end
        ...
        traceback.print_exc()
        raise RuntimeError(i18n("Audio loading failed"))
...
def check_for_existance(file_list:list=None,is_train=False,is_dataset_processing=False):
    ...
    if sum(files_status)!=len(files_status):
        if is_train:
            ...
            gr.Warning(i18n('file or folder does not exist'))
            return False
            ...
            if file_list[0]:
                ...
                gr.Warning(i18n('Path cannot be empty'))
            return False
    return True
...
def check_details(path_list=None,is_train=False,is_dataset_processing=False):
    if is_dataset_processing:
        ...
        if (not list_path.endswith('.list')):
            gr.Warning(i18n('Please fill in the correct List path'))
            return
        if audio_path:
            if not os.path.isdir(audio_path):
                gr.Warning(i18n('Please fill in the correct audio folder path'))
                return
    ...
    if is_train:
        ...
        with open(phone_path,'r',encoding='utf-8') as f:
            if f.read(1):...
            else:gr.Warning(i18n('Lack of phoneme datasets'))
        if os.listdir(hubert_path):...
        else:gr.Warning(i18n('Missing Hubert dataset'))
        if os.listdir(wav_path):...
        else:gr.Warning(i18n('Lack of audio datasets'))
        df = pd.read_csv(
            semantic_path, delimiter="\t", encoding="utf-8"
        )
        if len(df) >= 1:...
        else:gr.Warning(i18n('Lack of semantic datasets'))
