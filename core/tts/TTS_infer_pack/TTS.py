...
sys.path.append(os.path.join(now_dir, "core", "tts"))   # utils 모듈을 찾기 위해 경로 추가
import ffmpeg
import librosa
import numpy as np
import random
...
from core.tts.AR.models.t2s_lightning_module import Text2SemanticLightningModule
from core.tts.feature_extractor.cnhubert import CNHubert
from core.tts.module.models import SynthesizerTrn
from core.tts.module.mel_processing import spectrogram_torch
from core.tts.tools.i18n.i18n import I18nAuto, scan_language_list
from core.tts.tools.my_utils import load_audio
from transformers import AutoModelForMaskedLM, AutoTokenizer
from core.tts.TTS_infer_pack.text_segmentation_method import splits
from core.tts.TTS_infer_pack.TextPreprocessor import TextPreprocessor

language=os.environ.get("language","Auto")
language=sys.argv[-1] if sys.argv[-1] in scan_language_list() else language
i18n = I18nAuto(language=language)
LIBRARY_NAME = "core/tts"
...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str]=None):
        ...
        # Load Model_weights - TODOs: if you want to use Custom tools don't activate this
        ...
        self.sampling_rate:int = 36000  # 32000 Default
    ...
    def _load_configs(self, configs_path: str)->dict:
        if os.path.exists(configs_path):
            ...
            print(i18n("경로가 존재하지 않습니다. 기본 구성을 설정하세요"))
...
class TTS:
    ...
    def _set_prompt_semantic(self, ref_wav_path:str):
        ...
        with torch.no_grad():
            ...
            if (wav16k.shape[0] > 160000 or wav16k.shape[0] < 48000):
                raise OSError(i18n("참조 오디오가 3~10초 범위를 벗어났습니다. 변경해 주세요!"))
    ...
    def run(self, inputs:dict):
        ...
        if parallel_infer:
            print(i18n("병렬 추론 모드 활성화"))
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_batch_infer
        else:
            print(i18n("병렬 추론 모드 비활성화"))
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_naive_batched

        if return_fragment:
            print(i18n("세그먼트 리턴 모드 활성화"))
            if split_bucket:
                split_bucket = False
                print(i18n("분할 반환 모드는 버킷 처리를 지원하지 않으며 버킷 처리가 자동 비활성화."))

        if split_bucket and speed_factor==1.0:
            print(i18n("버킷 처리 모드 활성화"))
        elif speed_factor!=1.0:
            print(i18n("음성 속도 조절은 버킷 처리를 지원하지 않으며, 버킷 처리가 자동 비활성화"))
            split_bucket = False
        else:
            print(i18n("버킷 처리 모드 비활성화"))

        if fragment_interval<0.01:
            fragment_interval = 0.01
            print(i18n("세그먼트 간격이 너무 작아서 자동으로 0.01로 설정되었습니다."))
        ...
        if not no_prompt_text:
            ...
            print(i18n("Reference text actually entered:"), prompt_text)
        ...
        try:
            ...
            for item in data:
                ...
                print(i18n("Text processed by the front end (per sentence):"), norm_text)
                if no_prompt_text :
                    prompt = None
                else:
    ...
    def run_generator(self, inputs:dict):
        ...
        if not (len(list(paths)) == len(aux_ref_audio_paths) == len(self.prompt_cache["aux_ref_audio_paths"])):
            ...
            for path in aux_ref_audio_paths:
                if path in [None, ""]:
                    continue
                if not os.path.exists(path):
                    print(i18n("오디오 파일이 존재하지 않습니다. 건너뛰기：{}").format(path))
                    continue
                self.prompt_cache["refer_spec"].append(self._get_ref_spec(path))
        ...
        if not no_prompt_text:
            prompt_text = prompt_text.strip("\n")
            if (prompt_text[-1] not in splits): prompt_text += "。" if prompt_lang != "en" else "."
            print(i18n("실제로 입력된 참조 텍스트:"), prompt_text)
            if self.prompt_cache["prompt_text"] != prompt_text:
                self.prompt_cache["prompt_text"] = prompt_text
                self.prompt_cache["prompt_lang"] = prompt_lang
                phones, bert_features, norm_text = \
        ...
        try:
            ...
            for item in data:
                ...
                print(i18n("프론트 엔드에서 처리되는 텍스트(문장당):"), norm_text)
                if no_prompt_text :
                    prompt = None
                else:
                    prompt = self.prompt_cache["prompt_semantic"].expand(len(all_phoneme_ids), -1).to(self.configs.device)
                ...
                from core.tts.TTS_infer_pack.zero_crossing import find_zero_zone, find_matching_index
                zc_index1 = 0
                zc_index2 = 0
